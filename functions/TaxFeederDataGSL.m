...
function [data] = TaxFeederDataGSL(file_to_extract,region,voltageRegulatorSetting)
...
elseif (strcmp(file_to_extract,'R5-12.47-1.glm')~=0)    
    ...
    data.nom_volt = 7967.43; % GL changed to make it inline with line ratings
    data.feeder_rating = 1.15*9.473; 
    data.avg_house = 5000; % changed to define case studies better
    data.avg_commercial = 20000;
    ...
    % GL added: modifies the voltage setting based on whether we want to
    % force issues or not
    if voltageRegulatorSetting == 1
        regSetPoint = data.nom_volt;
    elseif voltageRegulatorSetting == 2
        regSetPoint = data.nom_volt-300;
    else 
       error('Wrong regulator setting') 
    end
    data.voltage_regulation = {regSetPoint; % band center (V)
        75; % band width (V)
        30; % delay (s)
        16; % num taps in one direction (-)
        .1};% p.u. regulation capacity in one direction (-)
...
elseif (strcmp(file_to_extract,'R5-12.47-4.glm')~=0)    
    data.nom_volt = 7200; % GL changed to make it inline with line ratings
    data.feeder_rating = 1.15*7.612; 
    data.avg_house = 3000; % GL changed to define cases for testing
    data.avg_commercial = 30000;
...
elseif (strcmp(file_to_extract,'R5-25.00-1.glm')~=0)    
    data.nom_volt = 13221.32;
    data.feeder_rating = 1.15*12.346; 
    data.avg_house = 6000/2; % GL changed to define the cases 2 uses transient only overloads
    ...
    if voltageRegulatorSetting == 1
        regSetPoint = data.nom_volt;
    elseif voltageRegulatorSetting == 2
        regSetPoint = 1.05*data.nom_volt;
    else 
       error('Wrong regulator setting') 
    end
    
    % this structure defines the voltage regulator settings for control. We
    % determine the setpoint for the regulator above based on the option we
    % use to define no issues (1) or borderline issues (2). 
    data.voltage_regulation = {regSetPoint; % band center (V)
        132; % band width (V)
        30; % delay (s)
        16; % num taps in one direction (-)
