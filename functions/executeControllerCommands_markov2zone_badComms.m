...
function [m_vect] = executeControllerCommands_markov2zone_badComms(controllerInformation, num_zones, timeUntilUnlocked, T_min, T_max, onOffSignal, theta_a, m_vect, logical_index_high, logical_index_low, locked, useLocalVoltage, tclVoltage)
%{
...
u2_ON_val = onOffSignal(1:2:end-1,1:N1*N2);
u3_ON_val = onOffSignal(1:2:end-1,N1*N2+1:2*N1*N2);
u4_ON_val = onOffSignal(1:2:end-1,2*N1*N2+1:3*N1*N2);
u2_OFF_val = onOffSignal(1:2:end-1,3*N1*N2+1:4*N1*N2);
u3_OFF_val = onOffSignal(1:2:end-1,4*N1*N2+1:5*N1*N2);
u4_OFF_val = onOffSignal(1:2:end-1,5*N1*N2+1:6*N1*N2);
...
%Not yet vectorized
for j = 1:N_houses
    if dPowerIdx == 1 %Switch more TCLs ON
        ...
        if (1 <= prev_aggr_state(j)) && (prev_aggr_state(j) <= N1*N2)  %Check if the TCL is OFF
            if ismember(prev_aggr_state(j),controllerInformation.binModel.rq2_OFF_idx) %Switch to 1 OFF 2 ON
               if (randomValueAtTcl(j) <= u2_ON_val(j,prev_aggr_state(j))) && ((q1_prev(j) == 0) && (q2_prev(j) == 0)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 0;
                   q2_ctrl_temp(j) = 1;
               end
            elseif ismember(prev_aggr_state(j),controllerInformation.binModel.rq3_OFF_idx) %Switch to 1 ON 2 OFF
               if (randomValueAtTcl(j) <= u3_ON_val(j,prev_aggr_state(j))) && ((q1_prev(j) == 0) && (q2_prev(j) == 0)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 1;
                   q2_ctrl_temp(j) = 0;
               end
            elseif ismember(prev_aggr_state(j),controllerInformation.binModel.rq4_OFF_idx) %Switch to 1 ON 2 ON
               if (randomValueAtTcl(j) <= u4_ON_val(j,prev_aggr_state(j))) && ((q1_prev(j) == 0) && (q2_prev(j) == 0)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 1;
                   q2_ctrl_temp(j) = 1;
               end                    
            end
        ...
        if (N1*N2+1 <= prev_aggr_state(j)) && (prev_aggr_state(j) <= 2*N1*N2)  %Check if the TCL is 1 OFF and 2 ON
            if ismember(prev_aggr_state(j)-N1*N2,controllerInformation.binModel.rq1_ON_FROM_Q2_idx) % check if the current temperature bin for (0,1) is controllable
               if (randomValueAtTcl(j) <= u2_OFF_val(j,prev_aggr_state(j)-N1*N2)) && ((q1_prev(j) == 1) || (q2_prev(j) == 1)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 0;
                   q2_ctrl_temp(j) = 0;
               end
            end
        elseif (2*N1*N2+1 <= prev_aggr_state(j)) && (prev_aggr_state(j) <= 3*N1*N2)  %Check if the TCL is 1 ON and 2 OFF
            if ismember(prev_aggr_state(j)-2*N1*N2,controllerInformation.binModel.rq1_ON_FROM_Q3_idx) % check if the current temperature bin for (1,0) is controllable
               if (randomValueAtTcl(j) <= u3_OFF_val(j,prev_aggr_state(j)-2*N1*N2)) && ((q1_prev(j) == 1) || (q2_prev(j) == 1)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 0;
                   q2_ctrl_temp(j) = 0;
               end
            end
        elseif (3*N1*N2+1 <= prev_aggr_state(j)) && (prev_aggr_state(j) <= 4*N1*N2)  %Check if the TCL is 1 ON and 2 ON
            if ismember(prev_aggr_state(j)-3*N1*N2,controllerInformation.binModel.rq1_ON_FROM_Q4_idx) %% check if the current temperature bin for (1,1) is controllable
               if (randomValueAtTcl(j) <= u4_OFF_val(j,prev_aggr_state(j)-3*N1*N2)) && ((q1_prev(j) == 1) || (q2_prev(j) == 1)) %Switch if the random value is greater
                   q1_ctrl_temp(j) = 0;
                   q2_ctrl_temp(j) = 0;
               end
            end
